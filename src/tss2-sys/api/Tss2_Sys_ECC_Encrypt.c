    ...
    const TPMT_KDF_SCHEME *inScheme)
{
    ...
    if (platinText) {
        rval = Tss2_MU_TPM2B_MAX_BUFFER_Marshal(platinText, ctx->cmdBuffer,
                            ctx->maxCmdSize, &ctx->nextData);
    } else {
        ctx->decryptNull = 1;
    ...
    TPM2B_DIGEST *c3)
{
    ...
    rval = Tss2_MU_TPM2B_ECC_POINT_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize,
                                &ctx->nextData, c1);
    if (rval)
        return rval;

    rval = Tss2_MU_TPM2B_MAX_BUFFER_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize,
                                &ctx->nextData, c2);
    if (rval)
        return rval;
    return Tss2_MU_TPM2B_DIGEST_Unmarshal(ctx->cmdBuffer,
                                ctx->maxCmdSize, &ctx->nextData, c3);
}
